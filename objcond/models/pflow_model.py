...
class PflowModel(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.track_and_cell_encoder = MPNN(config['embedding model'])
        self.global_encoder = MPNN(config['embedding model'])
    ...
    def infer(self,g):
        with torch.no_grad():
            self(g)
            if self.config["output model type"] == "Set2Set" or self.config['output model type']=='combined': #or self.config['output model type']=='combined':
                predicted_particles, predicted_num_particles = self.outputnet.infer(g) #, predicted_attention_weights = self.outputnet.infer(g)
